...
class AppRouter {
  ...
  Router get router {
    ...
    // Feature Routes
    // Since AppRouter is mounted at /api, these will become /api/auth, /api/customers, etc.
    // authRoutes mounts `/login`, so we mount it at `/auth` -> `/api/auth/login`
    router.mount('/auth', setupAuthRoutes(authController).call);
    
    // The other routes already include their prefixes (e.g. `/customers`) internally,
    // so we just mount them at the root of AppRouter -> `/api/customers`
